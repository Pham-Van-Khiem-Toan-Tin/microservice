...
//        CategoryEntity category = categoryRepository.findById(productEntities.get(0).getCategoryId()).orElseThrow(
//                () -> new BusinessException(VALIDATE_FAIL)
//        );
//        BrandEntity brand = brandRepository.findById(productEntities.get(0).getBrandId()).orElseThrow(
//                () -> new BusinessException(VALIDATE_FAIL)
//        );
//
//        List<ProductDTO> productDTOS = productEntities.stream().map(
//                pr -> ProductDTO.builder()
//                        .id(pr.getId())
//                        .name(pr.getName())
//                        .slug(pr.getSlug())
...
//            kafkaTemplate.send("catalog.product.created", savedProduct.getId(), event);
//            System.out.println("✅ Đã lưu MongoDB thành công: " + savedProduct.getId());
//        } catch (Exception e) {
...
//                List<String> skuImageIds = skus.stream().map(s -> s.getImage().getImagePublicId()).toList();
