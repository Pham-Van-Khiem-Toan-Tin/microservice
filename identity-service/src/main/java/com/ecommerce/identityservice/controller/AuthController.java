...
import com.ecommerce.identityservice.dto.response.AuthDTO;
...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<String>> register(@RequestBody RegisterForm registerForm) {
        ...
        return ResponseEntity.ok(ApiResponse.ok(EMAIL_VERIFY, userEntity.getEmail()));
    }
    ...
    public ResponseEntity<ApiResponse<String>> resendOtp(@RequestBody ResendOtpForm resendOtpForm) throws MessagingException {
        ...
        return ResponseEntity.ok(ApiResponse.ok(RESEND_OTP_SUCCESS));
    }
...
//        OAuth2User user = oat.getPrincipal();
        Set<String> permissions = authentication.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toSet());
        String userId = firstNonNull(jwt.getClaimAsString("uid"),     // nếu có
                jwt.getSubject());
